  ...
  patterns: [
    {
      ...
      to: path.resolve(__dirname, "prod", "manifest.json"),
    },
  ],
});

let copyIcons: (sizenums: number[], outdir: string) => CopyPlugin = (
  sizenums,
  outdir
) => {
...
const plugins: (
  | ((this: webpack.Compiler, compiler: webpack.Compiler) => void)
  | webpack.WebpackPluginInstance
)[] = htmlWebpackPluginArray(["popup", "options"])
  .concat(copyManifestJson)
  .concat(copyIcons(iconsSize, "prod"));
...
const rules: webpack.RuleSetRule[] = [ts, scss, elm];
